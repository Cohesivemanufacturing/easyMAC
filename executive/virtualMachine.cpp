...
static inline int parse_block(char *block, char code)
{
	...
	{
		if (*cstr == code)
		{						   // if the char associated with the pointer pnt is equal to the code
...
static inline int *parse_H_variable(char *block)
{
	...
	if (temp[0] != 0)
	{ // If we need to change any parameter
		if (temp[0] == 102)
		{ // Specific for changing origin
...
VirtualMachine::VirtualMachine(Winsocket *socket) : m_logFileBaseName("./out/motion_log"), // Base name for log files, now with directory
													m_logFileIndex(0),
													m_currentFileSize(0),
													cloudNCsocket(socket) // Initialize cloudNCsocket here
{
	DeleteExistingLogFiles(); // Call the deletion function
...
VirtualMachine::VirtualMachine(const std::string &fileName, Display *display, Shader *shader, Camara *camara) : m_logFileBaseName("./out/motion_log"), // Base name for log files, now with directory
																												m_logFileIndex(0),
																												m_currentFileSize(0),
	...
	while (parameters.xtarget - outx > m_actTol || parameters.ytarget - outy > m_actTol || parameters.ztarget - outz > m_actTol)
	{
		...
		if (qf >= m_fmax)
		{
			...
			if (qx >= parameters.steplim || qy >= parameters.steplim ||
				qz >= parameters.steplim)
			{ // if the q register of one of these DDAs overflowed
				if (qx >= parameters.steplim)
				{								  // if it was the x-register
					qx = qx % parameters.steplim; // reset the q-resister
					if (parameters.CB)
						parameters.py--; // if circulat bit is on (inversion bit IB calc. outside the loop)
					if (parameters.xtarget > outx)
					{														// if target not reached
						...
						if (stepDir.stepx->JointDir == -1)
							encoded_bit = encoded_bit | stepDir.stepx->directionBitValue;
					}
				}
				if (qy >= parameters.steplim)
				{								  // if it was the x-register
					...
					if (parameters.ytarget > outy)
					{
						...
						if (stepDir.stepy->JointDir == -1)
							encoded_bit = encoded_bit | stepDir.stepy->directionBitValue;
					}
				}
				if (qz >= parameters.steplim)
				{								  // if it was the z-register
					...
					if (parameters.ztarget > outz)
					{
						...
						if (stepDir.stepz->JointDir == -1)
							encoded_bit = encoded_bit | stepDir.stepz->directionBitValue;
					}
				}
			}
		}
		...
		if (m_isCloudNC)
		{
			...
			else
			{
				...
				{
					// Log the compression bit to the file
					std::stringstream ss;
					ss << "[" << compression_bit << "]";
					LogMotionData(ss.str()); // Log the compression bit value
					compression_bit = 0;	 // Reset the compression bit counter
				}

				std::stringstream decimalStream;
				decimalStream << std::setw(2) << std::setfill('0') << std::dec << encoded_bit;
				std::string decimalWord = decimalStream.str();

				// Log the motion data to the file
				LogMotionData(decimalWord);
			}
		}
	}
...
void VirtualMachine::DeleteExistingLogFiles()
{

	try
	{
		std::filesystem::path dir_path("./out/");
		for (const auto &entry : std::filesystem::directory_iterator(dir_path))
		{
			if (entry.is_regular_file())
			{
				...
				std::string baseName = m_logFileBaseName.substr(6); // Remove "./out/"
				std::string expectedFilenamePrefix = baseName + "_";

				// Check if the filename starts with the expected prefix
				if (filename.rfind(expectedFilenamePrefix, 0) == 0)
				{
	...
	{
		std::cerr << "Error deleting log files: " << ex.what() << std::endl;
	}
}
